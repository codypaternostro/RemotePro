function Update-RpConfigCommand {
    ...
    param (
        [Parameter(ParameterSetName = 'ShowDialog', Mandatory = $true)]
        [Parameter(ParameterSetName = 'ConfigurationItems', Mandatory = $true)]
        [string]$ModuleName,
        ...
        [Parameter(ParameterSetName = 'ConfigurationItems', ValueFromPipeline = $true)]
        [Parameter(ParameterSetName = 'NoDialog', ValueFromPipeline = $true)]
    ...
    if (-not $ParameterValues) {
        $ParameterValues = @{}
        foreach ($paramName in $commandDetails.Parameters.PSObject.Properties.Name) {
            $ParameterValues[$paramName] = $commandDetails.Parameters.$paramName.Value
        }
    }
    ...
    # Apply passed ParameterValues directly if GUI is not used
        ...
        foreach ($paramName in $ParameterValues.Keys) {
            if ($commandDetails.Parameters[$paramName]) {
                ...
                Write-Verbose "Parameter '$paramName' updated with value '$($ParameterValues[$paramName])'"
            } else {
